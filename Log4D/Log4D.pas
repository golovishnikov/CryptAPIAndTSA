...

{
  ...
  changes by Alexey Valuev
  - added %w, %a and %h to the pattern
  - added LockingModel property to TLogFileAppender (as in log4net.Appender.FileAppender)
}
...
const
  ...
  AppendOpt      = 'append';
  { Locking model for TLogFileAppender. }
  LockingModelOpt   = 'lockingModel';
...
{ Events ----------------------------------------------------------------------}
  ...
  public
    ...
    constructor Create(const Logger: TLogLogger;
      const Level: TLogLevel; const Message: string;
      const ErrorMessage: string; const ErrorClass: string;
...
{ Loggers ---------------------------------------------------------------------}
  ...
  public
    ...
    procedure Debug(const Message: TObject; const Err: Exception = nil);
      overload; virtual;
    procedure Debug(const Format: string; const Args: array of const;
      const Err: Exception = nil);
      overload; virtual;
    ...
    procedure Error(const Message: TObject; const Err: Exception = nil);
      overload; virtual;
    procedure Error(const Format: string; const Args: array of const;
      const Err: Exception = nil);
      overload; virtual;
    ...
    procedure Fatal(const Message: TObject; const Err: Exception = nil);
      overload; virtual;
    procedure Fatal(const Format: string; const Args: array of const;
      const Err: Exception = nil);
      overload; virtual;
    ...
    procedure Info(const Message: TObject; const Err: Exception = nil);
      overload; virtual;
    procedure Info(const Format: string; const Args: array of const;
      const Err: Exception = nil);
      overload; virtual;
    ...
    procedure Warn(const Message: TObject; const Err: Exception = nil);
      overload; virtual;
    procedure Warn(const Format: string; const Args: array of const;
      const Err: Exception = nil);
      overload; virtual;
    function TryMessage(const AFormat: string; const Args: array of const): String;
  end;
...
{ Appenders -------------------------------------------------------------------}
  ...
  { Send log messages to a file.
    ...
    # How to lock the file: ExclusiveLock (default), MinimalLock (acquires the file lock for each write) or InterProcessLock (provides cross-process file locking)
    # It's working only for Delphi XE and up
    log4d.appender.<name>.lockingModel=InterProcessLock
  }
  TLogFileAppender = class(TLogStreamAppender)
  private
    FAppend: Boolean;
    FFileName: TFileName;
    FLockingModel: string;
    function CreateStream(fileName: string; append: boolean; mode: word): TStream;
  protected
...
{$ENDIF}
    constructor Create(const Name, FileName: string;
      const Layout: ILogLayout; const Append: Boolean; const LockingModel: string;
      const fCreateStream: boolean); overload;
    procedure SetOption(const Name, Value: string); override;
    procedure SetLogFile(const Name: string; doNotTouchLock: boolean);
  ...
  public
    constructor Create(const Name, FileName: string;
      ...
      const LockingModel: string = 'ExclusiveLock');
      reintroduce; overload;
    procedure Init; override;
    destructor Destroy; override;
    property FileName: TFileName read FFileName;
    property OpenAppend: Boolean read FAppend;
    property LockingModel: string read FLockingModel;
  end;
  ...
  public
    constructor Create(const Name, FileName: string;
      const Layout: ILogLayout = nil; const Append: Boolean = True;
      const LockingModel: string = 'ExclusiveLock';
      const MaxFileSize: integer = DEFAULT_MAX_FILE_SIZE;
      const MaxBackupIndex: integer = DEFAULT_MAX_BACKUP_INDEX);
      reintroduce; virtual;
    procedure Init; override;
...
begin
  ...
  if assigned(Err) then
    Create(Logger, Level, Message, Err.Message, Err.ClassName, TimeStamp)
  else
    Create(Logger, Level, Message, '', '', TimeStamp);
end;
...
function TLogEvent.GetAppName: string;
begin
  Result := currentAppName;
end;

function TLogEvent.GetUserName: string;
begin
  Result := currentUserName;
end;

function TLogEvent.GetHostName: string;
begin
...
type
  TPatternPart = (ppText, ppLogger, ppClassName, ppDate, ppException,
    ...
    ppLevel, ppRuntime, ppThread, ppNDC, ppPercent, ppApplication, ppUserName, ppHostName);

const
  { These characters identify the types above. }
  PatternChars        = ' cCdeFlLmMnprtx%awh';
...
{ Compile the formatted string from the specified pattern and its parts.
  ...
  a - Application name. //by Alexey Valuev
  w - User name. //by Alexey Valuev
  h - Host name. //by Alexey Valuev
...
begin
  ...
  for Index := 0 to FPatternParts.Count - 1 do
    case TPatternPart(FPatternParts.Objects[Index]) of
      ...
      ppApplication:
        Result := Result + Event.AppName;
      ppUserName:
        Result := Result + Event.UserName;
      ppHostName:
        Result := Result + Event.HostName;
...
procedure TLogStreamAppender.Init;
begin
  inherited;
  FStream := nil;
end;
...
constructor TLogFileAppender.Create(const Name, FileName: string;
  const Layout: ILogLayout; const Append: Boolean; const LockingModel: string);
begin
  Create(Name, FileName, Layout, Append, LockingModel, true);
end;

constructor TLogFileAppender.Create(const Name, FileName: string;
  const Layout: ILogLayout; const Append: Boolean; const LockingModel: string;
  const fCreateStream: boolean);
begin
  inherited Create(Name, nil, Layout);
  FLockingModel := LockingModel;
  FAppend := Append;
  if fCreateStream then
    SetOption(FileNameOpt, FileName)
  else
    FFileName := FileName;
end;

procedure TLogFileAppender.Init;
begin
  inherited;
  FLockCounter := 0;
  FLockingModel := 'ExclusiveLock';
  FAppend := false;
  FFileName := '';
{$IFDEF DELPHI10_UP}
  FMutex := nil;
...
destructor TLogFileAppender.Destroy;
begin
{$IFDEF DELPHI10_UP}
  if FMutex <> nil then
    FMutex.Free;
...
function TLogFileAppender.CreateStream(fileName: string; append: boolean; mode: word): TStream;
var
  strPath: string;
  f : TextFile;
begin
  ...
  begin
    ...
    strPath := ExtractFileDir(fileName);
...
{ create file stream }
procedure TLogFileAppender.SetLogFile(const Name: string; doNotTouchLock: boolean);
var
  fileName: string;
{$IFDEF DELPHI10_UP}
  mutextFriendlyFilename: string;
{$ENDIF}
begin
  EnterCriticalSection(FCriticalAppender);
  try
    if doNotTouchLock then
    begin
      if FStream <> nil then
        FreeAndNil(FStream);
    end
    ...
    if FFileName <> '' then
    begin
      if FLockingModel = 'ExclusiveLock' then
        FStream := CreateStream(FFileName, FAppend, fmOpenReadWrite or fmShareDenyWrite)
      else
      begin
        if FLockingModel = 'MinimalLock' then
          FStream := nil
        else
          FStream := CreateStream(FFileName, FAppend, fmOpenReadWrite or fmShareDenyNone);
{$IFDEF DELPHI10_UP}
        if not doNotTouchLock then
        begin
          mutextFriendlyFilename := FFileName;
          mutextFriendlyFilename := mutextFriendlyFilename
            .Replace('\', '_')
            .Replace(':', '_')
            .Replace('/', '_');
          try
            FMutex := TMutex.Create(nil, false, mutextFriendlyFilename);
          except
            on e: EOSError do
              if e.ErrorCode = 5 then
                FMutex := TMutex.Create(SYNCHRONIZE, false, mutextFriendlyFilename);
          end;
        end;
{$ENDIF}
      end;
...
procedure TLogFileAppender.DoAppend(const msg: string);
begin
  EnterCriticalSection(FCriticalAppender);
  try
    if FFileName <> '' then
...
procedure TLogFileAppender.AcquireLock;
begin
{$IFDEF DELPHI10_UP}
  ...
  begin
    if FLockingModel = 'MinimalLock' then
      FStream := CreateStream(FFileName, FAppend, fmOpenReadWrite or fmShareDenyNone);
    if (FLockingModel = 'InterProcessLock') or (FLockingModel = 'MinimalLock') then
      FMutex.Acquire;
  end;
  Inc(FLockCounter);
{$ENDIF}
...
procedure TLogFileAppender.ReleaseLock;
begin
{$IFDEF DELPHI10_UP}
  Dec(FLockCounter);
  if FLockCounter = 0 then
...
begin
  ...
  try
    ...
    begin
      ...
      SetLogFile(FFileName, false);
    end
    else if (Name = FileNameOpt) and (Value <> '') then
    begin
      SetLogFile(Value, false);    // changed by adasen
    end
    else if (Name = LockingModelOpt)
      and ((Value = 'ExclusiveLock') or (Value = 'InterProcessLock')) then
    begin
{$IFDEF DELPHI10_UP}
      //ExclusiveLock or InterProcessLock
      FLockingModel := Value;
{$ENDIF}
      SetLogFile(FFileName, false);
    end;
...
begin
  ...
  try
    ...
    try
{$IFDEF DELPHI10_UP}
      if assigned(FStream) and ((FCurrentSize = 0) or (FLockingModel = 'InterProcessLock') or (FLockingModel = 'MinimalLock')) then
{$ELSE}
...
constructor TLogRollingFileAppender.Create(const Name, FileName: string;
  const Layout: ILogLayout = nil; const Append: Boolean = True;
  const LockingModel: string = 'ExclusiveLock';
  const MaxFileSize: integer = DEFAULT_MAX_FILE_SIZE;
  const MaxBackupIndex: integer = DEFAULT_MAX_BACKUP_INDEX);
begin
  inherited Create(Name, FileName, Layout, Append, LockingModel, false);
...
begin
  ...
  try
    ...
    begin
      ...
      // close file
      if LockingModel = 'InterProcessLock' then
      begin
        {$IFDEF DELPHI10_UP}
        if CopyFile(PWideChar(FFileName), PWideChar(FFileName + '.1'), true) then
        {$ELSE}
        if CopyFile(PChar(FFileName), PChar(FFileName + '.1'), true) then
        {$ENDIF}
          FStream.Size := 0;
      end
      else
...
procedure TLogLogger.Debug(const Format: string;
  const Args: array of const; const Err: Exception);
begin
  Debug(TryMessage(Format, Args), Err);
end;

procedure TLogLogger.Error(const Format: string;
  const Args: array of const; const Err: Exception);
begin
  Error(TryMessage(Format, Args), Err);
end;

procedure TLogLogger.Fatal(const Format: string;
  const Args: array of const; const Err: Exception);
begin
  Fatal(TryMessage(Format, Args), Err);
end;

procedure TLogLogger.Info(const Format: string; const Args: array of const;
  const Err: Exception);
begin
  Info(TryMessage(Format, Args), Err);
end;

procedure TLogLogger.Warn(const Format: string; const Args: array of const;
  const Err: Exception);
begin
  Warn(TryMessage(Format, Args), Err);
end;

function TLogLogger.TryMessage(const AFormat: string;
...
begin
  try
    ...
    begin
      Result := 'Invalid format string: ' + AFormat + CRLF + Ex.Message;
      Error(Result);
    end;
  end;
end;
...
{$ENDIF}
  ...
  currentAppName :=
  {$IFDEF DELPHI10_UP}
    TPath.GetFileName(GetModuleName(0));
  {$ELSE}
    ExtractFileName(GetModuleName(0));
  {$ENDIF}
  currentHostName := GetComputerNetName;

  { Inicialize logger via LogInterface}
  Log4DReconfig;
finalization
